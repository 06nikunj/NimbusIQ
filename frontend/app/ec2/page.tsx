...
.stat-val{font-size:22px;font-weight:700;font-family:'IBM Plex Mono',monospace;color:#16191f;line-height:1}
...
.table-filter{padding:8px 16px;border-bottom:1px solid #d5dbdb;display:flex;align-items:center;gap:8px;background:#fafafa;flex-wrap:wrap}
.filter-input{border:1px solid #d5dbdb;border-radius:3px;padding:5px 10px;font-size:12px;width:220px;font-family:'Noto Sans',sans-serif;color:#16191f;outline:none}
.filter-input:focus{border-color:#0073bb}
.filter-select{border:1px solid #d5dbdb;border-radius:3px;padding:5px 10px;font-size:12px;font-family:'Noto Sans',sans-serif;color:#16191f;outline:none;background:white;cursor:pointer}
.aws-table{width:100%;border-collapse:collapse}
.aws-table th{font-size:11px;font-weight:700;color:#545b64;text-transform:uppercase;letter-spacing:.6px;padding:8px 12px;text-align:left;background:#fafafa;border-bottom:1px solid #d5dbdb;white-space:nowrap}
...
.warn-tag{display:inline-flex;align-items:center;gap:4px;background:#fef6e4;border:1px solid #f5d97a;border-radius:3px;padding:2px 6px;font-size:10px;color:#8d6605;margin-top:3px}
...
.cpu-bar{height:6px;background:#e8eced;border-radius:2px;width:80px;overflow:hidden}
.cpu-fill{height:100%;border-radius:2px}
.cpu-good{background:#1d8102}
.cpu-warn{background:#ff9900}
.cpu-bad{background:#d13212}
.cpu-num{font-family:'IBM Plex Mono',monospace;font-size:12px;color:#545b64;min-width:34px}
...
export default function EC2Page() {
  ...
  const totalCost = instances.length * 0.013

  function getCPUClass(cpu: number) {
    if (cpu < 5) return 'cpu-bad'
    if (cpu < 40) return 'cpu-good'
    return 'cpu-warn'
  }

  function getCPUColor(cpu: number) {
    if (cpu < 5) return '#d13212'
    if (cpu < 40) return '#1d8102'
    return '#ff9900'
  }
    ...
    <>
      ...
      <div className="main">
        <div className="page-header">
          <div>
            <div className="page-title">EC2 Instances</div>
            <div className="page-sub">Amazon Elastic Compute Cloud · ap-south-1 · {instances.length} total instances</div>
          </div>
        ...
        <div className="stats-row">
          <div className="stat-card">
            ...
            <div>
              ...
              <div className="stat-val" style={{ color: '#1d8102' }}>{loading ? '-' : running}</div>
            </div>
          </div>
          <div className="stat-card">
            <div className="stat-icon" style={{ background: '#fdf3f1' }}>🔴</div>
            <div>
              <div className="stat-label">Stopped</div>
              <div className="stat-val" style={{ color: '#d13212' }}>{loading ? '-' : stopped}</div>
            </div>
          </div>
          <div className="stat-card">
            ...
            <div>
              ...
              <div className="stat-val" style={{ color: '#0073bb' }}>{loading ? '-' : instances.length}</div>
            </div>
          </div>
          <div className="stat-card">
            ...
            <div>
              ...
              <div className="stat-val" style={{ color: '#ec7211' }}>${loading ? '-' : totalCost.toFixed(3)}</div>
            </div>
          </div>
        </div>
        ...
        <div className="panel">
          ...
          <div className="table-filter">
            <input className="filter-input" placeholder="Search by name or instance ID..." value={filter} onChange={e => setFilter(e.target.value)} />
          ...
          {loading ? (
            <div className="loading-box"><div className="loading-spin"></div>Fetching EC2 instances from AWS...</div>
          ) : error ? (
            <div className="empty-box">
              <div className="empty-icon">⚠️</div>
              <div className="empty-title">Could not load instances</div>
            ...
            <div className="empty-box">
              ...
              <div className="empty-sub">
                {instances.length === 0 ? 'No EC2 instances found. Connect your AWS credentials in Settings.' : 'No instances match your filter.'}
              </div>
              {instances.length === 0 && (
                <button className="btn btn-primary" onClick={() => router.push('/settings')}>Connect AWS</button>
              )}
            </div>
          ) : (
              ...
              <tbody>
                {filtered.map(instance => (
                  <tr key={instance.id} style={{ background: instance.cpu < 5 && instance.state === 'running' ? '#fff8f8' : 'white' }}>
                    <td><input type="checkbox" checked={selected.includes(instance.id)} onChange={e => setSelected(e.target.checked ? [...selected, instance.id] : selected.filter(s => s !== instance.id))} /></td>
                    <td>
                      <div className="col-name">{instance.name}</div>
                      <div className="col-id">{instance.id}</div>
                      {instance.cpu < 5 && instance.state === 'running' && <div className="warn-tag">CPU at {instance.cpu}% - possible waste</div>}
                    </td>
                    <td><span className="col-mono">{instance.type}</span></td>
                    <td>
                      ...
                      {!['running','stopped','pending'].includes(instance.state) && <span className="badge badge-gray"><span className="bdot"></span>{instance.state}</span>}
                    </td>
                    <td><span className="col-mono">{instance.region}</span></td>
                    <td><span className="col-mono" style={{ fontSize: 11 }}>{new Date(instance.launchTime).toLocaleDateString('en-GB')}</span></td>
                    <td>
                      <div className="cpu-wrap">
                        <div className="cpu-bar"><div className={`cpu-fill ${getCPUClass(instance.cpu)}`} style={{ width: `${Math.min(instance.cpu, 100)}%` }}></div></div>
                        <span className="cpu-num" style={{ color: getCPUColor(instance.cpu) }}>{instance.cpu}%</span>
                      </div>
                    </td>
                    <td><span className="col-mono">$0.013</span></td>
          ...
          <div className="table-footer">
            <span>{selected.length > 0 ? `${selected.length} selected` : `Showing ${filtered.length} of ${instances.length} instances`}</span>
      ...
      <div className="footer">NimbusIQ - EC2 Monitor - ap-south-1 - 2026</div>
    </>
  )
}
